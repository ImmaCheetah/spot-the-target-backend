...
describe("Map routes", () => {
  test("returns status code 200 and correct map name", async () => {
    const response = await request(app).get("/map/2");
  ...
  test("returns status code 404 on bad route", async () => {
    const response = await request(app).get("/map/bad");
  ...
  test("creates start time on map load", async () => {
    const response = await request(app).post("/map/1");
...
describe("Leaderboard routes", () => {
  test("returns status 200 when getting leaderboard", async () => {
    const response = await request(app).get("/leaderboard/map/2");
  ...
  test("returns correct start time", async () => {
    ...
    const response = await request(app).get(`/leaderboard/${startId.id}`);
  ...
  test("returns 404 status if wrong route", async () => {
    const response = await request(app).get(`/leaderboard/wrong`);
